<script lang="ts">
    ...
    import type {Magazine} from "../../lib/specific_sources/magazine";
    ...
    let source: Source & { source_specific: Magazine } = data
        ? data.source as Source & { source_specific: Magazine }
        : {
    ...
    let source_authors = (source.source_specific as Magazine).authors;

    async function upload_source() {
        (source.source_specific as Magazine).authors = source_authors.filter((author) => author != "");
        if (source_id != "") {
